import cv2 as cv
import numpy as np
import Learning_Model_Test #--->Dont import until complete this program
from Learning_Model_Test import model
'''
Leaning_Model_Test is CNN (2_Hidden_Layer-->Inside : Conv & Max_Padding)
...
"""
x_1, y_1, x_2, y_2=50,50,300,300#ROI location-->200x200
...
while True:
    ...
    if key==27:#ESC
        ...
        test_img=cv.resize(test_img ,(320,240))
        cv.imshow("TestImg",test_img)
        test_img=np.array(test_img, dtype=np.float32)
        test_img=test_img.reshape((1,240,320,1))
